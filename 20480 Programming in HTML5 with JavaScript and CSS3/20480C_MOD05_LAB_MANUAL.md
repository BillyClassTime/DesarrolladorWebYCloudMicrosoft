...
```
Se le ha pedido que modifique la página ** Programación ** del sitio web ContosoConf. Anteriormente, los datos de la sesión se almacenaban como una matriz codificada y el código JavaScript de la página mostraba los datos de esta matriz. Sin embargo, la información de la sesión no es estática; Los organizadores de la conferencia pueden actualizarlo en cualquier momento y almacenarlo en una base de datos. Hay un servicio web disponible que puede recuperar los datos de esta base de datos y usted decide actualizar el código de la página ** Programación ** para usar este servicio web en lugar de los datos codificados actualmente incrustados en la aplicación.

Además, los organizadores de la conferencia preguntaron si era posible que los asistentes a la conferencia pudieran indicar a qué sesiones les gustaría asistir. Esto permitirá a los organizadores de la conferencia programar sesiones populares en salas más grandes. La página ** Programación ** se ha mejorado para mostrar iconos de estrellas junto a cada sesión. Un asistente puede hacer clic en el icono de una estrella para registrar su interés en esa sesión. Esta información debe registrarse en una base de datos en el servidor y usted envía esta información a otro servicio web que actualiza los datos correspondientes en la base de datos.

Para sesiones populares, el servicio web devolverá el número de asistentes que lo hayan seleccionado. Deberá manejar esta respuesta y mostrar un mensaje al asistente cuando haya seleccionado una sesión potencialmente ocupada.
```
