...
5. To display all the Microsoft Azure App Services, on the left menu panel, click **App Services**. (Directamente Web app)
...
6. To create  a new web app, enter details in the following fields:
    - In the **App Name** box, type the following web app name **mod6demo3** **{YourInitials}**. (.NET 5 (Early Access))
...
7. Open a new tab in the browser and navigate to https://www.visualstudio.com/team-services, and then click **Get Azure DevOps free**. **[NUEVO](https://dev.azure.com/)**
...
11. In the **MyApp** page, click **Repos** on the left blade then click **Generate Git credentials**, and then enter the following details: (**CLONAR VS CODE**)
     - Alias: **mod6demo3{YourInitials}** (NO)
     - Password: **Password123** (NO)
     - Confirm Password: **Password123** (NO)
       - {NO}
     - Click **Save Git Credentials**.  ... (NO)

12. In the left blade, click **Pipelines**. 

13. Click **New pipeline**.

     - Seleccionar "Crear una nueva carpeta "
     - MyApp (Saltar 27)
...
29. to create a new solution, run the following command: (NO) Añadir después el fichero .gitignore e ignorar bin y obj
     ```bash
     dotnet new sln -n Mod6Demo3
     ```

30. To add the **MyApp** project to the **Mod6Demo3** solution, run the following command: (NO)
     ```bash
...
33. To push the changes to our repository, run the following command:
 ...
 - Azure Repos
   ...
   - Azure Pipelines (Preview) Continue
       ...
       - Project->MyApp
       - Repository->MyApp
       - Branch->Master ->Continue ->Finish git push
     ```
...
9. To add credentials to the app service, under the **DEPLOYMENT** section, click **Deployment center**, in the open page select **FTP**, click **Dashboard** and then enter the following information:
    ...
    - In the **Password** and **Confirm password** boxes, type **P@ssword99**.
