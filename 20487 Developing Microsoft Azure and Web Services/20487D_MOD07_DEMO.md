...
Es posible que se requiera un clean del certificado y renovacion
dotnet dev-certs https --clean
dotnet dev-certs https --trust

[System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12;
Invoke-WebRequest -Uri https://localhost:5001/api/reservation/createvoucher -ContentType "application/json" -Method POST -Body "'Billy'"

Confirmar que el ApiWeb esta ok creando un nuevo metodo HelloWorld
 [HttpGet("Get")]
 public ActionResult GetReservation()
 {
 	return Content("Hello World");
 }
Desde un navegador o desde postman: (En Postman en el Request->Body->raw y JSON)
https://localhost:5001/api/reservation/get

O desde PWS:
Invoke-WebRequest -Uri https://localhost:5001/api/Reservation/Get -Method GET
   ```
...
15. Click **Add unique key**, and then type **customerId**. OJO la Id en Mayuscula
16. Click **Ok**.
17. On the top bar, click **New Collection**.
18. In **Database id**, select **Use existing**, and then select **mydb**.
19. In **Collection id**, type **orders**. (ShareKey name) (por los datos que vamos a alojar)
20. Click **Ok**.
...
#### Demonstration Steps (Follow the readme in folder, skip this demo)

1. Open Microsoft Edge.
2. Go to **https://portal.azure.com** and sign in with your credentials.
3. In the left pane, click **Azure Cosmos DB**. NO
4. On the top bar, click **Add**.
...
8. Click **Review + Create** and then click **Create**.
   - Quick  Start
     - .Net Creating a sample database with graph
     - Follow the Quickstart: Build a .NET Framework or Core application using the Azure Cosmos DB Gremlin API account
9. Wait for the database to be created.
...
13. In **Graph Id**, type **taskManager**
    1. Partition Key: **_sink**
    2. Click **OK**.
...
21. Copy the **primary endpoint** value for later use.
   ...
   >
   >https://mode7demo5bmvb.z22.web.core.windows.net/
...
26. In **Pricing tier**, select **Premium Verizon**. (**standard para cuentas trial**)
...
10. Copy **StackExchange.Redis**, which is the primary connection string, for later use.
    ...
    - ``` 
      mod7demo6redisbmvb.redis.cache.windows.net:6380,password=s+ccJcsOMxOOQt8rphhygZ6c+SUj5wf5i0ioaKWMuhQ=,ssl=True,abortConnect=False
      ```
