...
2. To create a new **ASP.NET Core Web API** project, at the command prompt, paste the following command, and then press Enter:
    ```bash
    dotnet new webapi --name BlueYonder.Flights.Service --output .\Starter\BlueYonder.Flights.Service -f netcoreapp2.1
    ```
...
1. On the **Start** menu, search for and right-click **OJO PowerShell 5**, and then select **Run as Administrator**.
...
5. In your **Web App** menu, in the **MONITORING** menu, click **Diagnostics logs**.
    ...
    Monitoring->App Services Log>Application Loggin(FileSystem):
...
11. Copy the **FTP host name** value to any code editor.
     ...
     ftp://waws-prod-par-015.ftp.azurewebsites.windows.net/site/wwwroot
...
1. To create a new **container** instance using the previously created **Docker container** for the hotel's service, run the following command: (Si cluster-windows: --os-type Windows)
    ```bash
...
```
az container create --resource-group blueyonder-hotels --name hotels-container-bmvb --image billyclasstime/hotels_service2021 --dns-name-label blueyonder-hotels-service-bmvb --ports 80 

billyclasstime/hotels_service2021
```

1. To get details of your container instance, run the following command, and then press Enter:
    ```bash
    az container list
    
    az container logs --resource-group blueyonder-hotels --name hotels-container-{Your Initials}
    ```
2. Inside the **"ipAddress"** object, locate the **"ip"** property and copy its value.
3. Paste the **IP address** in your browser's address bar, add **/api/hotels** to it, and then press Enter.
    >**Note**: Your URL should look like the following:<br/>
               {Container's IP Adress}**/api/hotels**
4. Check that you are getting the expected response just as the following:
    ```json
    [{"id":1,"hotelName":"Leonardo","address":"Jones Street 259, Manhattan","stars":5.0,"isFullyBooked":false},{"id":2,"hotelName":"Dan","address":"Bleecker Street 23, Manhattan","stars":3.5,"isFullyBooked":true}]
    ```
